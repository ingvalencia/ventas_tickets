...
elseif ($tipo === 'elimina_registros_carga_global') {
    if (!isset($data['fechaMin']) || !isset($data['fechaMax'])) {
        echo json_encode(["success" => 0, "error" => "Parámetros faltantes: fechaMin o fechaMax"]);
        exit;
    }
    ...
    // Ejecutar DELETE en lotes pequeños para evitar llenar el transaction log
    ...
    do {
        ...
        if (!$result) {
            echo json_encode(["success" => 0, "error" => "Error al eliminar registros: " . mssql_get_last_message()]);
            exit;
        }

        // Contar registros eliminados en este lote
        $rowsAffected = mssql_rows_affected($mysqli);
        $totalDeleted += $rowsAffected;
    } while ($rowsAffected > 0); // Repetir hasta que no queden registros

    echo json_encode(["success" => 1, "message" => "Se eliminaron $totalDeleted registros correctamente."]);
    exit;
}
...
elseif (isset($_POST['tipo']) && $_POST['tipo'] === 'subir_reporte_ticket_fragmento') {
    ...
    foreach ($registros as $datos) {
        ...
        if ($fechaVenta === false) {
            $errores++;
            $erroresDetalles[] = "Error en fecha de venta en registro: " . json_encode($datos);
            continue;
        }
        ...
        if (count($loteRegistros) === $registrosPorLote) {
            $resultado = insertarLote($loteRegistros, $debug_message);
            $insertados += $resultado['insertados'];
            $errores += $resultado['errores'];
            $erroresDetalles = array_merge($erroresDetalles, $resultado['detalles_errores']);
...
function insertarLote($loteRegistros) {
    ...
    if (!$result) {
        $debug_message .= "Fallo al insertar lote: " . mssql_get_last_message() . "\n";
        return ["success" => 0, "insertados" => 0, "errores" => count($loteRegistros), "detalles_errores" => [mssql_get_last_message()]];
    }

    $debug_message .= "Inserción exitosa de " . count($loteRegistros) . " registros\n";
    return ["success" => 1, "insertados" => count($loteRegistros), "errores" => 0];
}
